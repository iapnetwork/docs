...
This separation of concerns defines the architecture of the IAP Stack, which contains three separate layers: The Client for processing sensitive data, the Core for assuring data immutability, and Chains for processing large public compute sets, each of which is explained in detail in the following chapters. Currently, the IAP concerns itself with four different aspects of data proof: **Proof of existence**, **proof of process**, **proof of computation** and **proof of device state**. These first building blocks are referred to as the Information Assurance Toolbelt (IAT), which is fully extensible by the open source community. The IAP employs modern cryptographic methods within the construct of the Stack to achieve trustless proofs for each of the tools.
